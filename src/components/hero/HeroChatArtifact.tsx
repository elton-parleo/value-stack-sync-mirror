...
const StatusHeader = ({ phase }: { phase: Phase }) => {
  ...
  return (
    <motion.div
      className="relative flex items-center justify-between overflow-hidden border-b px-4 py-3"
      animate={{
    ...
    >
      <motion.div
        ...
        animate={{ x: phase === "scan" ? ["0%", "430%"] : "430%", opacity: phase === "scan" ? [0, 1, 0.25] : 0 }}
        transition={{ duration: 1.85, ease }}
      />
      <div className="relative flex min-w-0 items-center gap-2">
        <motion.span
          className="relative h-2 w-2 rounded-full"
          animate={{ backgroundColor: active ? "hsl(var(--primary))" : "hsl(var(--foreground) / 0.34)" }}
          transition={{ duration: 0.45 }}
        >
          <motion.span
            className="absolute inset-0 rounded-full bg-primary"
            animate={{ opacity: active ? [0.45, 0] : 0, scale: active ? [1, 2.7] : 1 }}
            transition={{ duration: 1.7, repeat: active ? Infinity : 0, ease: "easeOut" }}
          />
        </motion.span>
        <div className="min-w-0">
          <p className="font-label text-foreground/45">
            {active ? "Parleo layer active" : "Standard agent view"}
          </p>
          <p className="truncate text-[12.5px] font-medium text-foreground/70">
            {active ? "Incentives converted into machine-readable price" : "Ranking from visible sticker prices only"}
          </p>
        </div>
      </div>
      <motion.div
        className="relative hidden h-7 w-[128px] overflow-hidden rounded-full border border-border/70 bg-card/80 sm:block"
        animate={{ borderColor: active ? "hsl(213 99% 50% / 0.26)" : "hsl(var(--border) / 0.7)" }}
        transition={{ duration: 0.6 }}
      >
        <motion.div
          className="absolute inset-y-1 left-1 rounded-full bg-primary/12"
          animate={{ width: active ? "86%" : "34%" }}
          transition={{ duration: 0.95, ease }}
        />
        <span className="relative flex h-full items-center justify-center text-[10.5px] font-semibold uppercase tracking-[0.1em] text-foreground/55">
...
const ProductHeader = () => (
  <div className="flex items-center gap-3 px-4 pt-4">
    <div className="flex h-[58px] w-[58px] shrink-0 items-center justify-center overflow-hidden rounded-lg border border-border/70 bg-secondary/45">
      <img src={tatchaWaterCream} alt="Tatcha The Water Cream" className="h-full w-full object-contain" />
    </div>
    <div className="min-w-0 flex-1">
      <p className="text-[10.5px] font-semibold uppercase tracking-[0.13em] text-foreground/45">Tatcha</p>
      <p className="truncate text-[14.5px] font-semibold leading-tight text-foreground">The Water Cream</p>
      <p className="text-[12px] text-foreground/52">50ml moisturizer</p>
    </div>
    <div className="hidden rounded-full border border-border bg-card px-2.5 py-1 text-[11px] font-medium text-foreground/55 sm:block">
      4 retailers
    </div>
  </div>
);
...
const ValueRoute = ({ phase }: { phase: Phase }) => {
  ...
  return (
    <div className="relative mt-4 h-[96px] px-4">
      <div className="absolute left-[26px] right-[26px] top-[47px] h-px bg-border/80" />
      <motion.div
        className="absolute left-[26px] top-[47px] h-px bg-primary"
        animate={{ width: corrected ? "calc(100% - 52px)" : active ? "58%" : "18%", opacity: active ? 0.65 : 0.22 }}
        transition={{ duration: 0.95, ease }}
      />

      <div className="grid h-full grid-cols-[1fr_64px_1fr] items-center gap-2">
        <RetailerNode
          name="Amazon"
          logo="Amazon"
          label="List price"
        ...
        >
          <motion.div
            className="absolute h-10 w-10 rounded-full border border-primary/25 bg-primary/[0.055]"
            animate={{ scale: active ? [1, 1.14, 1] : 1 }}
            transition={{ duration: 1.7, repeat: active ? Infinity : 0, ease: "easeInOut" }}
          />
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="hsl(var(--primary))" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="relative">
            <path d="M3 12h15" />
        ...
        <RetailerNode
          name="Sephora"
          logo="Sephora"
          label={active ? "True cost" : "List price"}
          price={active ? 14.64 : 30}
          selected={active}
        />
      </div>
    </div>
...
const RetailerNode = ({
  ...
  <motion.div
    className="relative h-[78px] rounded-xl border px-3 py-2.5"
    animate={{
      y: selected ? -4 : 0,
  ...
  >
    <div className="flex items-center gap-2">
      <BrandLogo name={logo} size={17} />
      <span className="text-[13px] font-semibold text-foreground/82">{name}</span>
    </div>
    <div className="mt-2 flex items-end justify-between gap-2 tabular-nums">
      <span className="text-[10.5px] font-medium uppercase tracking-[0.1em] text-foreground/40">{label}</span>
      <AnimatedMoney value={price} className={selected ? "text-[16px] font-bold text-primary" : "text-[14px] font-semibold text-foreground/72"} />
    </div>
    <motion.span
      className="absolute -right-1.5 -top-1.5 rounded-full border border-card px-2 py-0.5 text-[9.5px] font-bold uppercase tracking-[0.09em]"
      animate={{
  ...
  return (
    <div className="relative mt-4 px-4">
      <motion.div
        ...
        animate={{ borderColor: active ? "hsl(213 99% 50% / 0.22)" : "hsl(var(--border) / 0.7)" }}
      ...
      >
        ...
        />
        <motion.div
          className="pointer-events-none absolute inset-y-0 left-0 w-[46%] bg-gradient-to-r from-transparent via-primary/12 to-transparent"
          animate={{ x: phase === "scan" ? ["-120%", "245%"] : "245%", opacity: phase === "scan" ? [0, 1, 0] : 0 }}
          transition={{ duration: 1.55, ease }}
        />

        <div className="relative grid grid-cols-[1fr_auto] gap-3 px-3 py-3">
          <div>
            ...
            <div className="mt-2 space-y-1.5">
              {items.map((item, index) => (
                <motion.div
                  key={item.label}
                  className="flex items-center justify-between gap-3 text-[12.5px] tabular-nums"
                  animate={{ opacity: active ? 1 : 0.34, x: active ? 0 : -4 }}
                  transition={{ delay: active ? index * 0.12 : 0, duration: 0.45, ease }}
                >
          ...
          <div className="flex w-[112px] flex-col justify-between rounded-lg border border-border/65 bg-card/85 p-2.5 text-right">
            <span className="text-[10.5px] font-medium uppercase tracking-[0.1em] text-foreground/42">True cost</span>
            <AnimatedMoney
              value={active ? 14.64 : 30}
              className={active ? "text-[22px] font-bold leading-none text-primary" : "text-[20px] font-bold leading-none text-foreground/58"}
            />
            <motion.span
...
const Recommendation = ({ phase }: { phase: Phase }) => {
  ...
  return (
    <div className="px-4 pb-4 pt-4">
      <motion.div
        className="relative min-h-[74px] overflow-hidden rounded-xl border px-3.5 py-3"
        animate={{
          borderColor: active ? "hsl(213 99% 50% / 0.22)" : "hsl(var(--border) / 0.68)",
          backgroundColor: active ? "hsl(213 99% 50% / 0.045)" : "hsl(var(--secondary) / 0.26)",
        }}
        transition={{ duration: 0.65, ease }}
      >
        <div className="flex items-start gap-3">
          <motion.div
            ...
            animate={{
              borderColor: active ? "hsl(213 99% 50% / 0.26)" : "hsl(var(--border))",
          ...
          >
            <motion.svg
              ...
              viewBox="0 0 24 24"
              fill="none"
              stroke={active ? "hsl(var(--primary-foreground))" : "hsl(var(--foreground) / 0.52)"}
              strokeWidth="2.3"
              strokeLinecap="round"
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-[12.5px] leading-relaxed text-foreground/66">
              {corrected
                ? "Sephora wins after loyalty, card-linked value and gift value are made readable to the agent."
                : active
                  ? "The visible price is being reconciled against the shopper's incentive layer."
                  : "The agent can only see sticker price, so it picks the lowest listed offer."}
            </p>
          </div>
        </div>
      </motion.div>
...
const AnswerCard = ({ phase }: { phase: Phase }) => (
  <motion.div
    ...
    animate={{ opacity: 1, y: 0 }}
    transition={{ duration: 0.55, ease }}
    className="h-[436px] overflow-hidden rounded-xl border border-border bg-card sm:h-[452px]"
  >
...
const HeroChatArtifact = () => {
  ...
  useEffect(() => {
    ...
    TIMELINE.forEach(({ at, phase: nextPhase }) => {
      timers.push(setTimeout(() => setPhase(nextPhase), at));
    });
    ...
    <div className="w-full" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
      <ChatChrome progress={progress}>
        <div className="space-y-4">
          ...
          <div className="flex items-start gap-3">
            <AssistantAvatar />
            <div className="relative min-h-[436px] min-w-0 flex-1 sm:min-h-[452px]">
              <motion.div
                className="absolute left-0 top-0 rounded-2xl rounded-tl-md bg-secondary/60 px-3 py-2"
                animate={{ opacity: phase === "thinking" ? 1 : 0, y: phase === "thinking" ? 0 : -4 }}
