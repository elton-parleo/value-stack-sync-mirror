...
export const AgentNetwork = () => (
  <motion.div {...reveal} className="relative mt-10 overflow-hidden rounded-lg border border-background/15 bg-background/5 p-5">
    <div className="absolute inset-x-0 top-1/2 h-px bg-primary/40" />
    <div className="relative mx-auto w-fit rounded-md border border-primary/60 bg-code-bg px-4 py-4">
...
const ProductRow = ({ image, name, note, price, bad = false }: { image: string; name: string; note: string; price: string; bad?: boolean }) => (
  <div className="grid grid-cols-[52px_1fr_auto] items-center gap-3 border-t border-current/10 py-3">
    ...
    <div><strong className="block text-xs leading-tight">{name}</strong><span className={`mt-1 inline-flex rounded-full px-2 py-0.5 font-mono text-[8px] uppercase ${bad ? 'bg-accent-warm/15 text-accent-warm' : 'bg-success/15 text-success'}`}>{note}</span></div>
...
export const CollapsedFunnel = () => (
  <motion.div {...reveal} className="mt-9">
    ...
    <div className="mt-6 overflow-hidden rounded-lg border border-foreground/15 bg-surface">
...
export const FeedProof = () => (
  <div className="mt-6 rounded-lg bg-code-bg p-5 text-background">
...
export const AuditPreview = () => <motion.div {...reveal} className="mt-10 overflow-hidden rounded-lg border-t-2 border-primary bg-background p-5 text-foreground"><div className="flex items-center justify-between"><div><p className="font-mono text-[9px] uppercase tracking-[0.12em] text-primary">Your audit · day 3</p><strong className="mt-2 block text-lg">Where revenue leaks</strong></div><ScoreRing/></div><div className="mt-5 grid grid-cols-3 divide-x border-y py-4 text-center">{[['8','Drifted'],['12','Invisible'],['40+','Queries']].map(([v,l])=><div key={l}><strong className="block text-xl">{v}</strong><span className="text-[9px] text-muted-foreground">{l}</span></div>)}</div></motion.div>;
