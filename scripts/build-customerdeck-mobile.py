...
function markWide(el, est){
  ...
  el.style.setProperty('width', Math.min(1560, Math.max(720, est))+'px', 'important');
  el.style.setProperty('min-width', Math.min(1560, Math.max(720, est))+'px', 'important');
  var sec=el.closest('section'), host=el.parentElement;
  while(host&&host!==sec&&host.clientWidth<300){host=host.parentElement;}
  (host&&host!==sec?host:el.parentElement).classList.add('m-scroll');
}
