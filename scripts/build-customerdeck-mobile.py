...
.m-slide section[data-screen-label] .m-keep{min-width:780px!important;width:780px!important}
...
document.querySelectorAll('.m-chapter').forEach(function(ch){
  ...
  if(sec && n!=='01'){
    ...
    if(row) row.classList.add('m-shead');
  }
...
document.querySelectorAll('.m-chapter section [style*="grid-template-columns"]').forEach(function(g){
  ...
  var t=g.style.gridTemplateColumns||'',n;
  var m=t.match(/repeat\((\d+)/);
  n=m?parseInt(m[1],10):t.trim().split(/\s+/).length;
  if(n<3)return;
  g.classList.add('m-keep');
  var host=g.parentElement;
  while(host&&host!==sec&&host.clientWidth<300){host=host.parentElement;}
  (host&&host!==sec?host:g.parentElement).classList.add('m-scroll');
});
...
document.querySelectorAll('.m-chapter section').forEach(function(sec){
  sec.querySelectorAll('*').forEach(function(el){
    ...
    if(fs&&fs<19){ el.style.fontSize=Math.max(20, fs*1.35)+'px'; el.style.lineHeight='1.35'; }
  });
});
